...
def _cancel_background_task_without_wait(task: asyncio.Task[None]) -> None:
    if task.done():
...
async def _warm_target_paper_seed_best_effort(
    seed: PaperSeed,
    *,
    discovery_client,
    github_client,
...
async def export_arxiv_relations_to_csv(
    ...
    *,
    ...
    target_warmup_task = asyncio.create_task(
        _warm_target_paper_seed_best_effort(
            target_seed,
            discovery_client=discovery_client,
            github_client=github_client,
            arxiv_client=arxiv_client,
    ...
    try:
        ...
        _cancel_background_task_without_wait(target_warmup_task)
        raise
