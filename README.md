...
- In single-paper relation mode, the input target paper also runs through the shared single-paper sync path as a best-effort attempt to warm reachable cache entries and gather cacheable information, but that warming run does not emit additional references/citations CSV rows
