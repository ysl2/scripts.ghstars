...
async def export_github_search_to_csv(
    ...
    *,
    ...
    input_adapter = GithubSearchInputAdapter()
    output_adapter = FreshCsvExportAdapter()
    rows = [
        output_adapter.to_csv_row(input_adapter.to_record(row), sort_index=index)
        for index, row in enumerate(
            sorted(repositories, key=lambda row: row.created, reverse=True),
            1,
        )
    ]
